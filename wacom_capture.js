...
function createConnection(){
  const m={device:null,saved:[],context:null,desired:null,accepted:false,points:0,previous:null,down:false,busy:false,view:null,paused:false,message:'Conecte la Wacom una vez para autorizar Chrome.',queue:Promise.resolve()};
  ...
  m.autoConnect=()=>enqueue(async()=>{
    ...
    if(devices.length===1)await open(devices[0]);else if(devices.length>1)message('Hay varias Wacom. Pulse Conectar para elegir una.');
  });
  m.connect=async()=>{
  ...
  m.repeat=()=>enqueue(async()=>{
    if(!m.device || !m.context || m.context!==m.desired || m.accepted)return;
    await send(33,[0]);clearInk();await showScreen();await send(33,[1]);message('Wacom lista. Repita la firma y pulse ACEPTAR.');
  });
  ...
  function pen(e){
    ...
    m.previous=[x,y];m.points++;update();
  }
  m.disconnect=()=>{m.paused=true;return enqueue(async()=>{await close();message('Wacom desconectada por el operador.');});};
  m.setContext=context=>{m.desired=context;return enqueue(sync);};
  navigator.hid?.addEventListener('connect',()=>{void m.autoConnect();});
  navigator.hid?.addEventListener('disconnect',e=>{if(e.device===m.device)void enqueue(async()=>{await close();message('USB desconectado. Se reconectará cuando vuelva a enchufar la Wacom.');});});
...
export default function({parentElement,data,setStateValue}){
  ...
  const view={emit:p=>setStateValue('signature',p),update:s=>{
    ...
    q('#clear').disabled=s.busy || !s.device || !s.context || s.accepted;
    q('#accept').disabled=s.busy || !s.device || !s.context || s.points<2 || s.accepted;
